...
describe("check token validity using claims", () => {
    test("checks valid token claims", async () => {
        ...
        let jwtStr = `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.${encodedPayload}.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c`;
        expect(isTokenStillValidClaimsBased(jwtStr)).toBe(true);
    });
    test("checks expired token claims", async () => {
        let jwtStr = `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE3Mzk3MTI0MzMsImV4cCI6MTczOTc1MjQzM30.1LOyxfn7rwwFyrKeVWnAVSt9GOohlZvMjUpykXGpzpI`;
